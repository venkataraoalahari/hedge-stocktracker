...
class TestPowerBIStreamingTask extends TimerTask {
	...
	String[] colList = {"symbol","timestampValue","dateValue","timeValue","histratio","currentratio","ratiodeviation","threshold","Alert"};
//	String[] colList = {"symbol", "count"};
	String finalOutput;

	public TestPowerBIStreamingTask(String[] args) {
	...
	public void executeQuery() {
		String finalQuery;
		String selectPart = "select symbol, "
							+ "substr(timestampValue,0,19) as timestampValue, "
							+ "substr(timestampValue,0,10) as dateValue, "
							+ "substr(timestampValue,12,8) as timeValue, "
							+ "histratio, currentratio, ratiodeviation, threshold, 1 as Alert "
							+ "from stock_alert ";
		...
		try {
			...
			while (rs.next()) {
				...
				for (int i = 0; i < colList.length; i++) {
					if ((colList[i] == "symbol") || (colList[i] == "timestampValue") || (colList[i] == "dateValue")
							|| (colList[i] == "timeValue"))
...
public class TestPowerBIStreaming {
	public static void main(String[] args) {
		TestPowerBIStreamingTask task = new TestPowerBIStreamingTask(args);
