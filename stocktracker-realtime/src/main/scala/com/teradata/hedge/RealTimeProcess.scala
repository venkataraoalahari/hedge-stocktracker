...
import com.teradata.hedge.RealTimeStreamingProcessing.{AlertGeneration, getDatafromCassandra}
...
object RealTimeProcess  extends Utils_scala {
    ...
    if (histValue != 99999.0){
      ...
      if (ratioDeviation != 99999.0) {
        if (ratioDeviation > threshold) {
          AlertGeneration(realTimeData, histValue, ratioDeviation, path)
  ...
  def AlertGeneration(tuple: (String, String, Double), histValue: Double, deviation: Double, path: String): Unit = {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val data = result.map(m =>
      StockMarket(m._1, m._2, m._3, m._4, m._5, m._6, m._7, m._8, m._9)
    )
    //  LOG.error("Aggregation completed ..")
    //  println("Inserted data into Ignite...")
    //data.print()
    //    val igniteConnection = new IgniteSinkImplement()
    //igniteConnection.InsertDataIntoIgnite(data)
